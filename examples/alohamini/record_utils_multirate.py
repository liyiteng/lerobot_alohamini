...
try:
    from .safety_utils import RecordingGate, SafetyRecorder, safety_snapshot
except ImportError:
    from safety_utils import RecordingGate, SafetyRecorder, safety_snapshot
...
class ControlSample:
    ...
    issued_command: dict[str, Any] = field(default_factory=dict)
...
class FreshCameraGate:
    ...
    def observe(self, timestamps: dict[str, float], *, now: float) -> float | None:
        ...
        stalled = [
            name for name in self.camera_names if now - self._last_advanced_at[name] > self.stall_timeout_s
        ]
        if stalled:
            raise RuntimeError(
                f"camera capture timestamp stalled for > {self.stall_timeout_s:.1f} s: {', '.join(stalled)}"
            )

        if not all(name in timestamps for name in self.camera_names):
            return None
        if not all(float(timestamps[name]) != self._last_recorded.get(name) for name in self.camera_names):
            return None
...
def _dataset_camera_names(robot: Any, dataset: LeRobotDataset | None) -> tuple[str, ...]:
    ...
    if unknown:
        raise ValueError(
            f"dataset requests cameras that are not configured on the robot: {', '.join(unknown)}"
        )
    return requested
...
def record_loop(
    ...
    wall-time overrun when camera delivery slows or temporarily stalls. Capture
    timestamps and gaps are retained in the episode sidecar.
    """
    ...
    sample_buffer = ControlSampleBuffer(max_samples=max(2, math.ceil(control_fps * sample_buffer_s)))
    camera_gate = (
        FreshCameraGate(
            expected_cameras,
    ...
    target_dataset_frames = max(1, round(control_time_s * fps)) if dataset is not None else None
    ...
    with SafetyRecorder(dataset) as safety_recorder:
        feedback_gate = RecordingGate(robot, dataset, safety_recorder)
        while True:
            ...
            if target_dataset_frames is None:
                ...
                break
            elif elapsed_s > max_recording_wall_time_s and not overrun_reported:
                print(
                    f"\nWaiting for fresh frames ({dataset_frames_written}/{target_dataset_frames}); "
                    "teleoperation continues. Press -> to save the collected frames.",
                    flush=True,
                )
            ...
            if not feedback_gate.state_ready():
                precise_sleep(max(0.0, control_interval - (time.perf_counter() - start_loop_t)))
                continue
            ...
            if robot.send_action(robot_action_to_send) == {}:
                precise_sleep(max(0.0, control_interval - (time.perf_counter() - start_loop_t)))
                continue
            ...
            current_sample = ControlSample(
                ...
                issued_command=dict(getattr(robot, "last_sent_command", {})),
            )
            ...
            if dataset is not None and not expected_cameras and state_only_sample_due:
                ...
                try:
                    ...
                    if camera_timestamp is not None:
                        ...
                        if not all(name in obs_processed for name in expected_cameras):
                        ...
                        if last_recovery_reason is not None:
                            logging.info("Camera/state alignment recovered; dataset recording continues")
                            safety_recorder.write(safety=sampled_safety, event={"type": "capture_recovered"})
                        last_recovery_reason = None
                except RuntimeError as exc:
                    selected_sample = None
                    recovery_reason = str(exc).partition(":")[0]
                    if recovery_reason != last_recovery_reason:
                        logging.warning("Waiting for aligned camera data: %s; teleoperation continues", exc)
                        safety_recorder.write(
                            safety=sampled_safety,
                            event={"type": "capture_wait", "reason": str(exc)},
                        )
            ...
            if dataset is not None and selected_sample is not None:
                ...
                observation_frame = build_dataset_frame(dataset.features, aligned_observation, prefix=OBS_STR)
                action_frame = build_dataset_frame(dataset.features, selected_sample.action, prefix=ACTION)
                dataset.add_frame({**observation_frame, **action_frame, "task": single_task})
                safety_recorder.write(
                    safety=selected_sample.safety,
                    requested_action=selected_sample.action,
                    issued_command=selected_sample.issued_command,
                    alignment_error_s=alignment_error_s if math.isfinite(alignment_error_s) else None,
                    host_timing={
                        "state_sample_monotonic_s": selected_sample.state_timestamp,
                        "camera_capture_monotonic_s": host_timing.get("camera_capture_monotonic_s", {}),
                    },
                )
            ...
            timing_totals_s["observation_processing"] += observation_processing_done_t - observation_done_t
        ...
        if dataset is not None and not exited_early and dataset_frames_written != target_dataset_frames:
            raise RuntimeError(
